...
client.on("guildMemberRemove", (member) => {
  ...
  guild.channels.find(channel => channel.name === "joins").send("Welcome to Sunset Hotels founded by AviaBays, " + member.user + "!\n Join our ROBLOX group: https://www.roblox.com/groups/4503579/Sunset-Hotels#!/about");});
   ...
   var guildid = message.guild.id
    db.fetch(`guildPrefix_${guildid}`).then(i => {
      ...
      let args = message.content.slice(prefix.length).trim().split(" ");
        ...
        try {
       //                    if (!message.member.roles.find(r => r.name === "Bot Tester")) return message.channel.send(":tools: The bot is in maintenance mode currently for: `Bug Fixes and minor speed improvements`, expected completion time: in 10 minutes. Only `Bot Testers` can use this bot in maintenance mode. ")  ;
